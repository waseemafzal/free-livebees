...

{
...

	{
		...
		extract($_POST);
		$notification_date=	date('d-m-Y', strtotime($notification_date));
$date=	date('d-m-Y', strtotime($date));

if($date<$notification_date){
    $message .= lang('observation_date_should_be_greator_than_notification_date') ;
    echo json_encode(array('status' => 204, 'message' => $message));exit;
}
$notificationDate=date('Y-m-d',strtotime($notification_date));
unset($_POST['notification_date']);
$this->db->where('id',$_POST['nest_id'])->update('tbl_loc',array('notification_date'=>$notificationDate,'next_observation_date'=>$date));
...
$aData['notification_date'] =$this->db->select('notification_date')->where('id',$id)->get('tbl_loc')->row()->notification_date;
...
*/
...

	{
		...
		$this->load->view('viewHistory', $htmlDATA);
...
/*$alarm_before_daye='- '.$alarm_before.' day';
$_POST['notification_date']=date('Y-m-d',(strtotime ( $alarm_before_daye , strtotime ( $next_observation_date) ) ));*/
$alarm_before_daye='- '.$alarm_before.' day';
$_POST['notification_date']=date('Y-m-d',strtotime($alarm_before));
...

	{
		...
		extract($_POST);
//observation_date_should_be_smaller_than_notification_date
$alarm_before=	date('d-m-Y', strtotime($alarm_before));
$next_observation_date=	date('d-m-Y', strtotime($next_observation_date));

if($next_observation_date<$alarm_before){
    $message .= lang('observation_date_should_be_greator_than_notification_date') ;
    echo json_encode(array('status' => 100, 'message' => $message));exit;
}
		if ($address == "") {
